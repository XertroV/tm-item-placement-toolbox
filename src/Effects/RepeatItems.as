namespace Repeat {
    ...
    void DrawRepeatTab(CGameCtnEditorFree@ editor) {
        ...
        UI::BeginTabBar("repeat methods");
        for (uint i = 0; i < methods.Length; i++) {
            auto method = methods[i];
            if (UI::BeginTabItem(method.name)) {
                methods[i].DrawControls(editor);
                UI::EndTabItem();
            }
        }
        UI::EndTabBar();
...
class GridRepeat : RepeatMethod {
    ...
    void DrawControls(CGameCtnEditorFree@ editor) override {
        ...
        if (UI::Button(Text::Format("Create %d Items", nbCreating))) {
            RunItemCreation(editor, Repeat::lastPicked.AsItem());
        }
    }
    ...
    mat4[]@ CalcPosRotMatricies() {
        ...
        vec3 maxPosIxs = Math::Max(vec3(grid_ItemsX, grid_ItemsY, grid_ItemsZ) - vec3(1), vec3(1));
